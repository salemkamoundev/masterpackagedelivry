                            ...
                            <div class="space-y-3">
                                @for (pass of trip.passengers; track $index) {
                                    <div class="border border-indigo-100 bg-indigo-50/50 rounded-xl p-4 flex justify-between items-center">
                                        ...
                                        <button (click)="togglePassengerDropoff(trip, $index)" 
                                                class="w-10 h-10 rounded-full flex items-center justify-center border-2 transition-all"
                                                [ngClass]="pass.isDroppedOff ? 'bg-indigo-600 text-white border-indigo-700' : 'bg-white text-gray-300 border-gray-300'">
                                            {{ pass.isDroppedOff ? '✓' : '' }}
                                        </button>
                                    </div>
                                }
...
export class DriverDashboardComponent {
  ...
  async submitNewTrip() {
      ...
      this.user$.pipe(switchMap(u => {
          ...
          // CORRECTION ICI : Ajout de 'passengers' qui manquait
          const newTrip: Trip = {
             ...
             passengers: val.passengers as any[] ?? [], // <--- LIGNE AJOUTÉE POUR CORRIGER TS2741
             extraRequests: []
          };
  ...
  // Implémentation simplifiée pour le script (si besoin d'extra logic, réintégrer)
  ...
  activeCount() { return 0; } // Placeholder pour éviter erreur template si pas utilisé
  setFilter(m: any) {} // Placeholder
