    ...
    <div class="min-h-screen flex items-center justify-center bg-gray-100 px-4">
      <div class="max-w-md w-full space-y-8 bg-white p-8 rounded-xl shadow-lg">
        ...
        <form [formGroup]="registerForm" (ngSubmit)="onSubmit()" class="mt-8 space-y-4">
         ...
         <div class="text-center mt-4"><a routerLink="/login" class="font-medium text-indigo-600 hover:text-indigo-500">Déjà un compte ? Se connecter</a></div>
      </div>
    </div>
  `
})
  ...
  onSubmit() {
    if (this.registerForm.valid) {
      ...
      this.authService.register(email!, password!, name!, role as any, company!, phoneNumber!).subscribe({
        next: () => { alert('Compte créé !'); this.router.navigate(['/login']); },
